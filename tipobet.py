...
#  KAYNAKLAR
# ─────────────────────────────────────────────
REDIRECT_URL = "https://zirveyonlendir.xyz/tipobettv"
...
OUTPUT_M3U = "tipobet.m3u"
OUTPUT_JSON = "tipobet_yayinlar.json"

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36",
    "Referer": "https://data-reality.com/",
    "Accept": "application/json, text/plain, */*"
}

def get_active_domain():
    """Yönlendirme üzerinden güncel domain'i bulur"""
    try:
        print(f"🔍 Aktif domain {REDIRECT_URL} adresinden alınıyor...")
        r = requests.get(REDIRECT_URL, headers=HEADERS, allow_redirects=True, timeout=10)
        if r.url:
            domain = r.url.rstrip('/')
            print(f"✅ Aktif domain bulundu: {domain}")
            return domain
    except Exception as e:
        print(f"⚠️ Domain bulma hatası: {e}")
    
    # Fallback
    return "https://tipobettv265.com"

def get_base_url():
    """Base URL'yi API'den alır"""
    try:
        print(f"📡 Base URL {DOMAIN_API_URL} adresinden alınıyor...")
    ...
    return "https://tzy.zirvedesin236.cfd/"
...
def create_m3u_content(matches, channels, base_url, active_domain):
    ...
    for m in matches:
        ...
        m3u_content.append(f'#EXTINF:-1 tvg-logo="{main_logo}" group-title="{group_title}",{display_name}')
...
def main():
    print("🚀 Tipobet TV M3U ve JSON Oluşturucu Başlatılıyor...")
